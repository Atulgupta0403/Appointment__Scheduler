...
const signUp = async (req, res) => {
    try {
        ...
        if (!firstName || !lastName || !email || !password || !username) {
            return res.json(new ApiResponse(505, "All fields are required" , false , false));
        }
    
        const user = await userModel.findOne({ $or: [{username}, {email} ] });
        if(password.length < 6){
            return res.json(new ApiResponse(200 , "Password must be greater than 6 letter"))
        }
    
        else if (user) {
            return res.json(new ApiResponse(200 , "Username or email already exist"));
        }
        else {
            bcrypt.genSalt(10, function (err, salt) {
                bcrypt.hash(password, salt, async function (err, hash) {
                    ...
                    return res.json(new ApiResponse(200, createdUser, " User created"))
                });
            });
        }
    } catch (error) {
        console.log("occur some error in signup " , error)
        res.send("occur some error in signup " , error);
    }
}
